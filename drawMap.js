...
//prototype addEventListener shorten it Element.on(event, fn)
Element.prototype.on = function(ev, fn, bubbles){  var b = (bubbles)?true:false; this.addEventListener(ev, fn, b); };
...
function init(){
    ...
    //Create stage enable mouse over
    var stage = new createjs.Stage(id);
    stage.enableMouseOver();
    createjs.Touch.enable(stage);
    //default vars 
    var fColor = "#9BBFDD", s = new createjs.Graphics(), w = "white"; defText = "We've Got You Covered", f = false, t = true, containers = [], stateShapes = [];
    //map json 
    var map = [
    ...
    function setCursor(arg){
        ...
        document.querySelector('body').style.cursor = arg;

    }
    // set blue label to selected state
    function setLabel(state){

        document.querySelector('.status-current-state').innerHTML = state;

    }

    setLabel(defText);
    var bg = new Image();
    bg.src = "images/mapShading.gif";

    bg.on('load', function(){

        document.querySelector('#'+id).style.backgroundImage = "url(images/mapShading.gif)"; 
    ...
    canImage.src = 'images/canadaTrans.png';

    canImage.on('load', function(){

        var canadaContainer = new createjs.Container();
        var can = new createjs.Bitmap('images/canadaTrans.png');
        ...
        canadaContainer.on('mouseover', function(){
           
            setLabel('Canada');
            setCursor('pointer');

            canadaContainer.alpha = .01;

        });
        
        canadaContainer.on('mouseout', function(){
        ...
        canadaContainer.on("click", function(){
        ...
        container.on('click', function(e){ 
            ...
            if(ev == 'touchstart'){
                ...
                if(conf){
                    ...
                    window.location.href = link + container.abr;
    ...
    function createBox(ob, index){
        ...
        containerBox.on('mouseover', function(){
            
            setCursor('pointer');
            buttonUp.alpha = .01;
            try {
                
                stateShapes[index].alpha = .01;
        ...
        containerBox.on('mouseout', function(){
    ...
    //DC 
    createBox({name:"District of Columbia", abr:'DC', txt:{x:606.313, y:234.59}, box:{x:600.313, y:234.59, n:0} }, null);
